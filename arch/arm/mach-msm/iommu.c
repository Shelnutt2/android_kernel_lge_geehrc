...
//Start LGE_BSP_CAMERA : iommu_patch - jonghwan.ko@lge.com
			msm_iommu_dump_pg(ctx_drvdata->attached_domain,GET_FAR(base, num));
...
int msm_iommu_dump_pg(struct iommu_domain *domain, unsigned long va)
{
	...
	fl_offset = FL_OFFSET(va);	/* Upper 12 bits (section)*/

	if (fl_offset >= 5)
		prt_offset = fl_offset - 5;
	else
		prt_offset = 0;
	/* Log 5 entries before and 5 entries after */
	for(; prt_offset < 4096 && prt_offset < (fl_offset+5); ++prt_offset) {
		unsigned long sl_addr;
		fl_pte = fl_table + fl_offset;
		sl_addr = (unsigned long) __va(((*fl_pte) & FL_BASE_MASK));

		if (prt_offset == fl_offset)
			pr_info("*[%lu]: 0x%08lx [%03lx]", prt_offset, sl_addr, 0UL);
		else
			pr_info(" [%lu]: 0x%08lx [%03lx]", prt_offset, sl_addr, 0UL);
	}
	...
	if (sl_offset >= 5)
		prt_offset = sl_offset - 5;
	else
		prt_offset = 0;
	/* Log 5 entries before and 5 entries after */
	for(; prt_offset < 256 && prt_offset < (sl_offset+5); ++prt_offset) {
		sl_pte = sl_table + prt_offset;

		phy = (*sl_pte & SL_BASE_MASK_SMALL);
		pte_flags = (*sl_pte & ~SL_BASE_MASK_SMALL);

		if (prt_offset == sl_offset)
			pr_info("*[%lu]: 0x%08lx [%03lx]", prt_offset, phy, pte_flags);
		else
			pr_info(" [%lu]: 0x%08lx [%03lx]", prt_offset, phy, pte_flags);
	}
