...
#if defined(CONFIG_USING_INNOTEK_PANEL_3_5)  /* L0 MPCS */
...
static struct timeval t_debug[2];
...
/* ESD mode change for D1LV and D1LK */
	...
	for (i = 0; i < read_num; i = i + 6) {
		if (ret < 0) {
			...
			printk("[TOUCH] touchType=%d", touchType);
			...
			if (touchType == TOUCH_SCREEN) {
				...
				printk("[TOUCH] TOUCH_KEY CLICKED!!");
...
mms136_intensity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < 12; i++) {
		...
		write_buf[0] = 0xBF;
		i2c_master_send(ts->client, write_buf, 1);
		i2c_master_recv(ts->client, read_buf, 22);
...
static int melfas_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (buf[2] == 'B') {
		...
		MELFAS_binary_nLength = MELFAS_binary_nLength_2; /* for 'C' */
		MELFAS_binary = MELFAS_binary_2;
		ts->pdata->fw_ver = 0x03;
	}
	...
	for (i = 0; i < I2C_RETRY_CNT; i++) {
		...
		if (ret >= 0) {
			...
			if (i == I2C_RETRY_CNT-1) {
				/* = mms100_download(check_type, embedded_img); //check FW status
				if (ret == 0) {
	...
	if (lge_get_board_revno() >= HW_REV_C) {
		if (buf[0] != ts->pdata->fw_ver && buf[2] != 'D') {
	...
	if (lge_get_board_revno() >= HW_REV_C) {
		if (buf[0] != ts->pdata->fw_ver) {   /* update only Rev C ITO case */
			MELFAS_TS_DPRINTK(MELFAS_TS_DEBUG_PROBE, KERN_INFO, "[TOUCH] : download start \n");
			mms100_download(isc_type, embedded_img);
