...
static void vfe32_start_common(struct msm_cam_media_controller *pmctl) /* LGE_CHANGE, patch for IOMMU page fault, 2012.09.06, jungryoul.choi@lge.com */
{
...
static int vfe32_zsl(struct msm_cam_media_controller *pmctl)
{
	...
	vfe32_start_common(pmctl); /* LGE_CHANGE, patch for IOMMU page fault, 2012.09.06, jungryoul.choi@lge.com */
	msm_camio_bus_scale_cfg(
...
static int Set_LCD_Brightness(int value)
{
    int fd;
	...
	struct msm_isp_cmd *cmd)
{
	...
	case VFE_CMD_START_RECORDING:
		...
		//Start LGE_BSP_CAMERA::seongjo.kim@lge.com LCD Brightness limit set
		...
		//End LGE_BSP_CAMERA::seongjo.kim@lge.com LCD Brightness down when camcording enter
		//End LGE_BSP_CAMERA::seongjo.kim@lge.com LCD Brightness limit set
		if (vfe32_ctrl->operation_mode &
			VFE_OUTPUTS_PREVIEW_AND_VIDEO)
			rc = vfe32_configure_pingpong_buffers(
...
static void vfe32_process_camif_sof_irq(void)
{
	...
	vfe32_ctrl->vfeFrameId++;
//LGE_UPDATE_S 0828 add messages to debug timeout error yt.jeon@lge.com
	if (vfe32_ctrl->vfeFrameId < 15) {
		pr_err("%s: SOF frame id %d\n", __func__, vfe32_ctrl->vfeFrameId);
	}
//LGE_UPDATE_E 0828 add messages to debug timeout error yt.jeon@lge.com
...
static void vfe32_process_error_irq(uint32_t errStatus)
{
	...
	if (errStatus & VFE32_IMASK_CAMIF_ERROR) { /* LGE_CHANGE, handle bus overflow error from VFE, 2012.09.09, jungryoul.choi@lge.com */
		pr_err("vfe32_irq: camif errors\n");
		reg_value = msm_camera_io_r(
				vfe32_ctrl->vfebase + VFE_CAMIF_STATUS);
		v4l2_subdev_notify(&vfe32_ctrl->subdev,
			NOTIFY_VFE_ERROR, (void *)NULL); /* LGE_CHANGE, handle bus overflow error from VFE, 2012.09.09, jungryoul.choi@lge.com */
		pr_err("camifStatus  = 0x%x\n", reg_value);
		vfe32_send_isp_msg(vfe32_ctrl, MSG_ID_VFE_ERROR); /* LGE_CHANGE, handle bus overflow error from VFE, 2012.09.09, jungryoul.choi@lge.com */
	}
...
/* LGE_CHANGE_S, handle bus overflow error from VFE, 2012.09.09, jungryoul.choi@lge.com */
...
static void vfe32_process_output_path_irq_0(void)
{
	...
	*/
	...
	if (out_bool) {
		...
		// Start LGE_BSP_CAMERA::seongjo.kim@lge.com Control camera kernel log
		logcount_nofreebuffer++;
		if (logcount_nofreebuffer > 30)
		{
			printk("path_irq_0 - no free buffer!\n");
			logcount_nofreebuffer = 0;
		}
		// End LGE_BSP_CAMERA::seongjo.kim@lge.com Control camera kernel log
	}
}
...
static void vfe32_process_output_path_irq_1(void)
{
	...
	if (out_bool) {
		...
		if (logcount_nofreebuffer > 30)
		{
			printk("path_irq_1 - no free buffer!\n");
			logcount_nofreebuffer = 0;
		}
		// End LGE_BSP_CAMERA::seongjo.kim@lge.com Control camera kernel log
	}
