...
struct synaptics_ts_platform_data {
	...
	//BSP_Touch_bringup 'enable change ldo_en at run-time'
	int (*power)(int on, bool log_on, int ldo_en);
...
//#define SYNAPTICS_RMI4_I2C_TIME_INFO_PRINT   (1)
...
//[LGE_Update_Start] 20111101 [Alpha] woosock.yang@lge.com
//BSP_touch_timecheck 'add debug function.'
#if defined(SYNAPTICS_RMI4_I2C_TIME_INFO_PRINT)
#define SYNAPTICS_TIME_MSG(fmt, args...) \
			printk(KERN_ERR "[Touch Time Info] [%s %d] " \
				fmt, __FUNCTION__, __LINE__, ##args);
#else
#define SYNAPTICS_TIME_MSG(fmt, args...)     do {} while (0);
#endif
//[LGE_Update_End] 20111101 [Alpha] woosock.yang@lge.com
